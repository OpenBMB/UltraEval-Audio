...
class AudioService:
    ...
    @staticmethod
    def read_audio_file(
        audio_path: str, max_duration: float = 29.9
    ) -> Optional[List[bytes]]:
        ...
        try:
            ...
            logger.info(
                f"Successfully processed {audio_path} into {len(audio_chunks)} chunk(s)"
            )
            return audio_chunks

        except Exception as exc:
            logger.error(
                f"Failed to read audio file {audio_path}: {exc}", exc_info=True
            )
            return None
        ...
        with requests.post(
            ...
            for line in response.iter_lines():
                ...
                try:
                    ...
                    line_content = (
                        choice_data["delta"] if stream else choice_data["message"]
                    )
                    ...
                    audio = (
                        [int(i) for i in StepAudioR1.audio_token_re.findall(audio)]
                        if audio
                        else None
                    )
    ...
    def apply_chat_template(self, messages):
        ...
        for message in messages:
            if message["role"] == "human" and isinstance(message["content"], list):
                processed = [
                    j for i in message["content"] for j in self.process_content_item(i)
                ]
