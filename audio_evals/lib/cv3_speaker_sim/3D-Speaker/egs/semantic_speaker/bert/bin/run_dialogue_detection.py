...
class DataTrainingArguments:
    ...
    task_name: Optional[str] = field(
        ...
        metadata={"help": f"The Dialogue-Detection Mission"},
    )

    max_seq_length: int = field(
        default=128,
        metadata={"help": f"The max_seq_length for training, default is 128"},
    )

    pad_to_max_length: bool = field(
        default=True,
        metadata={"help": f"Whether to pad all samples to `max_seq_length`"},
    )

    train_file: Optional[str] = field(
        default=None,
        metadata={"help": "A csv or a json file containing the train data."},
    )
    ...
    input_columns_name: Optional[str] = field(
        default="sentence", metadata={"help": f"The input key for columns name."}
    )

    output_columns_name: Optional[str] = field(
        default="label", metadata={"help": f"The output key for columns name."}
    )
...
def build_datasets_from_raw_datasets(
    model_args,
    data_args,
    training_args,
    raw_datasets,
...
def main_process_with_transformers(
    model_args,
    data_args,
    training_args,
    trainer: Trainer,
    ...
    if training_args.do_predict:
        ...
        sentence2label = {
            predict_dataset[index]["conversation_id"]: predict_dataset[index]["label"]
            for index, _ in enumerate(predict_dataset)
        }
        ...
        output_predict_simple_results_filename = os.path.join(
            training_args.output_dir, f"predict_simple_results.txt"
        )
        output_predict_json_file = os.path.join(
            training_args.output_dir, f"output_predict.json"
        )
        ...
        logger.info(
            f"***** Predict results conversation judge predict_dataset = {len(predict_dataset)}, "
        ...
        with open(output_predict_simple_results_filename, "w") as fw:
            for (
                ...
                sentence,
                prediction,
            ) in output_predict_simple_result_list:
...
def main():
    ...
    raw_datasets = build_raw_datasets(model_args, data_args, training_args)
