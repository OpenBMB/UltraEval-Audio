...
def main_process_with_transformers(
    ...
    if training_args.do_predict:
        ...
        for index in range(predict_num):
            ...
            predict_sentence, predict_speaker_change_list = (
                merge_split_sign_with_sentence(sentence, predict, label)
            )
            label_sentence, _ = merge_split_sign_with_sentence(sentence, label, label)

            logger.info(
                f"\npredict = {predict_sentence}" f"\nlabel   = {label_sentence}"
            )
    ...
    def compute_metrics(p):
        ...
        results = {
            "overall_precision": precision_score(
                concat_true_labels, concat_true_predictions, average=None
            )[1],
            ...
            "overall_accuracy": accuracy_score(
                concat_true_labels, concat_true_predictions
            ),
        }
        # results = {
