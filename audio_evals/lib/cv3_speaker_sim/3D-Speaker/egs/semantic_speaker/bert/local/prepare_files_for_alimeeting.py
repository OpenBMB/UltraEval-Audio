...
def get_args():
    parser = argparse.ArgumentParser(description="Prepare files for alimeeting dataset")
    parser.add_argument(
    ...
    for i in range(len(tg)):
        for j in range(len(tg[i])):
            ...
            if cur_seg.mark:
                ...
                rttm_list.append((utt_id, tg[i].name, cur_seg.minTime, cur_seg.maxTime))
...
def build_textgrid_wav_files(initial_path, trans7time_scp_file, trans7time_save_path):
    logger.info(
...
def main():
    ...
    train_near_trans7time_scp_file = os.path.join(
        save_path, "train_near_trans7time.scp"
    )
