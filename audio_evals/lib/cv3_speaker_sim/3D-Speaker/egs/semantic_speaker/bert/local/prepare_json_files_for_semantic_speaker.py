...
def get_args():
    parser = argparse.ArgumentParser(
        description="Prepare files for semantic speaker tasks"
    )
    parser.add_argument("--flag", default=None, help="The flag to diff")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=96,
        help="The sentence length for sliding windows",
    )
    parser.add_argument(
        "--sentence_shift",
        type=int,
        default=32,
        help="The sentence shift for sliding windows",
    )
...
def build_dialogue_detection_from_trans7time_shift_windows(
    utt_id: str, trans7time_list: List, sentence_length: int, sentence_shift: int
) -> List:
    ...
    while i < spk_sentence_num:
        ...
        while j < spk_sentence_num:
            ...
            role_tag_list.extend([cur_sentence_spk_index] * cur_sentence_length)
...
def build_speaker_turn_detection_from_trans7time_shift_windows(
    utt_id: str, trans7time_list: List, sentence_length: int, sentence_shift: int
) -> List:
    spk_sentence_list = split_trans7time(trans7time_list)
    spk_sentence_num = len(spk_sentence_list)
    ...
    while i < spk_sentence_num:
        ...
        while j < spk_sentence_num:
            ...
            cur_sentence_spk_index = spk_map[cur_sentence_spk_id]
            if (
                len(spk_label_list) != 0
                and spk_label_list[-1] != cur_sentence_spk_index
            ):
                change_point_list.append(count_sentence_length)
...
def main():
    ...
    for utt_id in tqdm.tqdm(trans7time_scp):
        ...
        dialogue_detection_results = (
            build_dialogue_detection_from_trans7time_shift_windows(
                utt_id, trans7time_list, sentence_length, sentence_shift
            )
        )
        speaker_turn_detection_results = (
            build_speaker_turn_detection_from_trans7time_shift_windows(
                utt_id, trans7time_list, sentence_length, sentence_shift
            )
        )
    ...
    dialogue_detection_json_file = os.path.join(
        save_path, f"{flag}.dialogue_detection.json"
    )
    speaker_turn_detection_json_file = os.path.join(
        save_path, f"{flag}.speaker_turn_detection.json"
    )
    write_json_file(dialogue_detection_json_file, total_dialogue_detection_results)
    write_json_file(
        speaker_turn_detection_json_file, total_speaker_turn_detection_results
    )
