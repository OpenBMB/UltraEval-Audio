...
def get_args():
    parser = argparse.ArgumentParser(description=f"Prepare files for aishell-4")
    parser.add_argument(
        "--home_path",
        required=True,
        help="The home path which including ['train_L', 'train_M', 'train_S', 'test']",
    )
    parser.add_argument("--save_path", required=True, help="The path to save files")
...
def filter_sentence(sentence: str) -> str:
    ...
    ), f"[{sentence}] have strange signals"
    return sentence
...
def filter_trans7time_list(utt_id, trans7time_list):
    ...
    for spk_id, st, ed, content in trans7time_list:
        ...
        if len(content) <= 0:
            continue
        result.append((spk_id, st, ed, content))
    return result
...
def main():
    ...
    for base_folder in tqdm.tqdm(base_folders):
        ...
        trans7time_scp = convert_textgrid_to_trans7time(
            cur_textgrid_scp, trans7time_save_path
        )
