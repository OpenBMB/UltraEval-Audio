    ...
    with open(args.out_scp, "w") as f:
        for idx, file in enumerate(files):
            ...
            for st in range(0, len(aud) - audlen, audstr):
                wavfile.write(
                    writedir + "/%05d.wav" % (st / fs), fs, aud[st : st + audlen]
                )
                f.write(
                    "{} {}\n".format(
                        "_".join(writedir.split("/")[-4:]),
                        writedir + "/%05d.wav" % (st / fs),
                    )
                )
            pbar.update(1)
    pbar.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--musan_dir", type=str, default="", help="musan dir")
    parser.add_argument("--out_scp", type=str, default="", help="out scp")
