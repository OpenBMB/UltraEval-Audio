...
def main():
    ...
    for segid in vad_json:
        ...
        while subseg_st + subseg_dur < ed:
            ...
            subsegid = (
                wavid + "_" + str(round(subseg_st, 2)) + "_" + str(round(subseg_ed, 2))
            )
        ...
        if subseg_st < ed:
            ...
            item = deepcopy(vad_json[segid])
            item.update({"start": round(subseg_st, 2), "stop": round(ed, 2)})
            subsegid = wavid + "_" + str(round(subseg_st, 2)) + "_" + str(round(ed, 2))
            subseg_json[subsegid] = item
