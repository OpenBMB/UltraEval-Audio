...
class VisionProcesser:
    def __init__(
        ...
        print(
            "video %s info: w: {}, h: {}, count: {}, fps: {}".format(
                w, h, self.count, self.fps
            )
            % self.video_id
        )
        ...
        self.speaker_detector = active_speaker_detection.ASDTalknet(
            onnx_dir, device, device_id
        )
        self.face_quality_evaluator = face_quality_assessment.FaceQualityAssess(
            onnx_dir, device, device_id
        )
        self.face_embs_extractor = face_recognition.FaceRecIR101(
            onnx_dir, device, device_id
        )

        # store facial feats along with the necessary information.
        self.active_facial_embs = {
            "frameI": np.empty((0,), dtype=int),
        ...
        if self.out_video_path is not None:
            ...
            self.v_out = cv2.VideoWriter(
                out_video_path, cv2.VideoWriter_fourcc(*"mp4v"), 25, (int(w), int(h))
            )
    ...
    def run(self):
        ...
        for [audio_sample_st, audio_sample_ed] in self.audio_vad:
            ...
            for _ in range(num_frames):
                ...
                if (index + 1) % self.shot_stride == 0:
                    audio = self.audio[
                        (frame_st + index + 1 - self.shot_stride)
                        * 640 : (frame_st + index + 1)
                        * 640
                    ]
                    self.process_one_shot(
                        frames,
                        face_det_frames,
                        audio,
                        frame_st + index + 1 - self.shot_stride,
            ...
            if len(frames) != 0:
                audio = self.audio[
                    (frame_st + index - len(frames)) * 640 : (frame_st + index) * 640
                ]
                self.process_one_shot(
                    frames, face_det_frames, audio, frame_st + index - len(frames)
                )
                frames, face_det_frames = [], []
        ...
        elapsed_time_msg = "The total processing time for %s is %.2fs, including" % (
            self.video_id,
            all_elapsed_time,
        )
        for k in self.elapsed_time:
    ...
    def process_one_shot(self, frames, face_det_frames, audio, frame_st=None):
        ...
        self.active_facial_embs["frameI"] = np.append(
            self.active_facial_embs["frameI"], active_facial_embs["frameI"] + frame_st
        )
        self.active_facial_embs["feat"] = np.append(
            self.active_facial_embs["feat"], active_facial_embs["feat"], axis=0
        )
        featTime = time.time()
    ...
    def face_detection(self, frames):
        ...
        for fidx, image in enumerate(frames):
            ...
            bboxes, _, probs = self.face_detector(
                image_input, top_k=10, prob_threshold=0.9
            )
    ...
    def track_shot(self, scene_faces):
        ...
        while True:  # continuously search for consecutive faces.
            ...
            for frame_faces in scene_faces:
                for face in frame_faces:
                    if track == []:
                        ...
                        face["frame"] - track[-1]["frame"] <= self.num_failed_det
                    ):  # the face does not interrupt for 'num_failed_det' frame.
            ...
            if track == []:
                ...
                len(track) > 1
                and track[-1]["frame"] - track[0]["frame"] + 1 >= self.min_track
            ):
                ...
                for ij in range(0, 4):
                    interpfn = interp1d(
                        frame_num, bboxes[:, ij]
                    )  # missing boxes can be filled by interpolation.
    ...
    def evaluate_fr(self, frames, tracks, scores):
        ...
        for tidx, track in enumerate(tracks):
            ...
            for fidx, frame in enumerate(track["track"]["frame"].tolist()):
                ...
                face = frames[frame][
                    max(int(bbox[1]), 0) : min(int(bbox[3]), frames[frame].shape[0]),
                    max(int(bbox[0]), 0) : min(int(bbox[2]), frames[frame].shape[1]),
                ]
                faces[frame].append(
                    {"track": tidx, "score": float(s), "facedata": face}
                )
        ...
        for fidx in range(len(faces)):
            ...
            if active_face_num == 1:
                ...
                active_facial_embs["frameI"] = np.append(
                    active_facial_embs["frameI"], fidx
                )
                active_facial_embs["feat"] = np.append(
                    active_facial_embs["feat"], feature, axis=0
                )
