...
parser.add_argument(
    "--visual_embs_dir", default=None, type=str, help="Visual embedding dir"
)
...
def audio_vision_func(
    local_wav_list, audio_embs_dir, visual_embs_dir, rttm_dir, config
):
    cluster = build("cluster", config)
    for wav_file in local_wav_list:
        wav_name = os.path.basename(wav_file)
        ...
        # cluster
        labels = cluster(
            audio_embeddings, visual_embeddings, audio_times, visual_times, config
        )
        # output rttm
        new_labels = np.zeros(len(labels), dtype=int)
        uniq = np.unique(labels)
