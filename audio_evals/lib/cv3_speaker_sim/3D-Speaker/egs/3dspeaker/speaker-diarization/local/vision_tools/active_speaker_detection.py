...
class ASDTalknet:
    ...
    def __init__(self, onnx_dir, device="cpu", device_id=0):
        ...
        assert os.path.exists(onnx_file_name), (
            "%s does not exist. Please check if it has been downloaded accurately."
            % onnx_file_name
        )
    ...
    def __call__(self, inputA, inputV):
        ort_inputs = {
            self.ort_net.get_inputs()[0].name: inputA,
            self.ort_net.get_inputs()[1].name: inputV,
        }
    ...
    def create_net(self, onnx_file_name, device="cpu", device_id=0):
        ...
        ort_session = onnxruntime.InferenceSession(
            onnx_file_name, options, providers=providers
        )
        return ort_session
