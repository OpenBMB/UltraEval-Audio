...
def prepare_csv(
    ...
    for audio_id in wav_keys:
        ...
        try:
            ...
            continue
        if fs != samplerate:
            str_log = "Error loading: unexpected file sample rate for %s: %d." % (
                wav_file,
                fs,
            )
            print(str_log)
...
def main(args):
    ...
    if os.path.exists(save_opt) and os.path.exists(save_csv):
        ...
        if conf_old != conf:
            raise ValueError(
                "An old different opt file is found. Please confirm and manually delete it."
            )
        else:
    ...
    if len(logs_tot) != 0:
        logger.warning(
            "WARNING: some loading errors occur and please check the %s" % save_log
        )
    ...
    with open(save_opt, "wb") as f:
        pickle.dump(conf, f)
    logger.info(
        "%s is prepared ready. It takes time %.2fs."
        % (args.data_dir, time.time() - start_time)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data/vox2_dev", help="train data dir"
    )
    parser.add_argument("--seg_dur", type=float, default=3.0, help="seg len")
    parser.add_argument("--sample_rate", type=int, default=16000, help="sample rate")
    parser.add_argument("--min_spkdur", type=float, default=0.0, help="min spk dur")
    parser.add_argument(
        "--random_seg", type=bool, default=False, help="crop the wav or not"
    )
    parser.add_argument("--nj", type=int, default=8, help="num of process")
