...
parser.add_argument(
    ...
    default="npy",
    type=str,
    help="Feat out format, npy or ark",
)
parser.add_argument("--batch_size", default=None, type=int, help="Batch size")
parser.add_argument(
    "--close_progress_bar", action="store_true", help="Close the progress bar"
)
...
def main():
    ...
    args.local_model_dir = os.path.join(
        args.local_model_dir, args.model_id.split("/")[1]
    )
    ...
    mp.spawn(
        main_process, nprocs=nprocs, args=(nprocs, args, wav_list, embedding_model)
    )
...
class IterWavList(IterableDataset):
    ...
    def chunk_wav(self, wav, chunk_sample_size):
        ...
        wavs = [
            wav[i * chunk_sample_size : (i + 1) * chunk_sample_size] for i in range(n)
        ]
