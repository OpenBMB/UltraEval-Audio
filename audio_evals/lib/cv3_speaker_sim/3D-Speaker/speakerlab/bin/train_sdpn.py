...
def main():
    ...
    if config.num_proto > 0:
        with torch.no_grad():
            ...
            _sqrt_k = (1.0 / config.output_dim) ** 0.5
            ...
            proto_labels = utils_rdino.one_hot(
                torch.tensor([i for i in range(config.num_proto)]), config.num_proto
            ).cuda()
    ...
    lr_schedule = utils_rdino.cosine_scheduler(
        config.lr * (config.batch_size_per_gpu * utils_rdino.get_world_size()) / 256.0,
        config.min_lr,
        config.epochs,
        len(train_loader),
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        if config.saveckp_freq and epoch % config.saveckp_freq == 0:
            utils_rdino.save_on_master(
                save_dict, os.path.join(model_save_path, f"checkpoint{epoch:04}.pth")
            )
        log_stats = {
...
def train_one_epoch(
    ...
    for it, data in enumerate(
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            # Add keleo loss
            ke_loss = sum(
                keleo_loss(student_output=p) for p in anchor_backbone_output.chunk(4)
            )
            loss = (
                ploss
                + config.memax_weight * me_max
                + config.koleo_loss_weight * ke_loss
            )
