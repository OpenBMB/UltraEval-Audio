...
parser.add_argument(
    "--resume", default=True, type=bool, help="Resume from recent checkpoint or not."
)
...
def train_process(config):
    ...
    for epoch in epoch_counter:
        ...
        if epoch % config.evaluate_epoch_freq == 0:
            val_stats = evaluate(
                val_dataloader, model, criterion, epoch, logger, config
            )
            if config.rank == 0:
                # log
                epoch_logger.log_stats(
...
def evaluate(val_loader, model, criterion, epoch, logger, config):
    ...
    progress = ProgressMeter(
        len(val_loader), val_stats, prefix="Epoch: [{}]".format(epoch)
    )
    ...
    with torch.no_grad():
        for i, (audioX, visualX, labels) in enumerate(val_loader):
            ...
            pred_scores = torch.cat(
                (pred_scores, pred_score.detach().reshape(-1).cpu())
            )
    ...
    mAP = average_precision(
        np.concatenate(pred_scores_list), np.concatenate(ground_labels_list)
    )
    key_stats = {
...
def test_process(config):
    ...
    # test
    key_stats = evaluate(
        test_dataloader, model, criterion, config.num_epoch, logger, config
    )
    if config.rank == 0:
