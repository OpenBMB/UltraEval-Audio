...
def get_args():
    parser = argparse.ArgumentParser(description="Export exist checkpoint to onnx file")
    parser.add_argument(
        "--experiment_path",
        required=True,
        type=str,
...
def build_model_from_modelscope_id(model_id: str, local_model_path):
    ...
    if model_id not in onnx_supports_dict:
        raise ValueError(f"model_id {model_id} is not currently onnx-supported")
    ...
    cache_dir = snapshot_download(model_id, revision=model_info_item["revision"])
...
def main():
    ...
    if model_id is not None:
        ...
        speaker_embedding_model = build_model_from_custom_work_path(experiment_path)
