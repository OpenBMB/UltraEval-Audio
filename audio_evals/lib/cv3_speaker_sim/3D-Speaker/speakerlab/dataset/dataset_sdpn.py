    ...
    def __getitem__(self, index):
        ...
        for j in range(self.local_num):
            local_audios_aug.append(
                self.augment_wav(local_audios[j], augment_profiles[j], "False")
            )
        ...
        if augment_profile["add_noise"] is not None:
            if glb_flag == "True":
                noiseaudio = fill_split(
                    augment_profile["add_noise"], self.max_frames, eval_mode=False
                )
            else:
                noiseaudio = fill_split(
            ...
            noise = (
                np.sqrt(
                    10 ** ((clean_db - noise_db - augment_profile["noise_snr"]) / 10)
                )
                * noiseaudio
            )
            audio = audio + noise
