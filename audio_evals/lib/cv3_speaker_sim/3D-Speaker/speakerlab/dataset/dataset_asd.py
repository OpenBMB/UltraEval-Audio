...
class TrainData(object):
    ...
    def __getitem__(self, index):
        ...
        audioSet = generate_audio_set(
            self.audio_dir, batchList
        )  # load the audios in this batch to do augmentation
        for line in batchList:
            data = line.split("\t")
            audioFeatures.append(
                load_audio(
                    data, self.audio_dir, numFrames, audioAug=True, audioSet=audioSet
                )
            )
            visualFeatures.append(
                load_visual(data, self.video_dir, numFrames, visualAug=True)
            )
...
class ValData(object):
    ...
    def __getitem__(self, index):
        ...
        audioFeatures = [
            load_audio(
                data, self.audio_dir, numFrames, audioAug=False, audioSet=audioSet
            )
        ]
        try:
            visualFeatures = [
                load_visual(data, self.video_dir, numFrames, visualAug=False)
            ]
        except:
            a = 1
