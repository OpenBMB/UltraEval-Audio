"""
VoxCPM: A Voice-Controlled Chinese Pre-trained Model for Speech Generationiz
...
class VoxCPMModel(nn.Module):
    def __init__(
        ...
        self.base_lm.setup_cache(
            1, config.max_length, self.device, get_dtype(config.dtype)
        )
    ...
    def generate(
        ...
        if len(prompt_wav_path) == 0:
            ...
            audio_mask = (
                torch.zeros(text_length).type(torch.int32).to(text_token.device)
            )
    ...
    def generate_with_prompt_cache(
        ...
        while retry_badcase_times < retry_badcase_max_times:
            ...
            if retry_badcase:
                ...
                break
        decode_audio = (
            self.audio_vae.decode(latent_pred.to(torch.float32)).squeeze(1).cpu()
        )
    ...
    def inference(
        ...
        feat_pred = feat_pred[..., :-1]  # trick: remove the last token
