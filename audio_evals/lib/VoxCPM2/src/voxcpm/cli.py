...
def load_model(args) -> VoxCPM:
    ...
    try:
        model = VoxCPM.from_pretrained(
            hf_model_id=getattr(args, "hf_model_id", "openbmb/VoxCPM"),
...
def cmd_batch(args):
    ...
    if args.prompt_audio:
        prompt_audio_path = str(
            validate_file_exists(args.prompt_audio, "reference audio file")
        )
...
Examples:
  ...
  voxcpm --text "Hello" --output out.wav --hf-model-id openbmb/VoxCPM
        """,
    )
    ...
    parser.add_argument(
        "--output", "-o", help="Output audio file path (single-sample mode)"
    )

    # Prompt audio (for voice cloning)
    parser.add_argument("--prompt-audio", "-pa", help="Reference audio file path")
    parser.add_argument(
        "--prompt-text", "-pt", help="Reference text corresponding to the audio"
    )
    parser.add_argument(
        "--denoise",
        action="store_true",
        help="Enable prompt speech enhancement (denoising)",
    )

    # Generation parameters
    parser.add_argument(
        "--cfg-value", type=float, default=2.0, help="CFG guidance scale (default: 2.0)"
    )
    parser.add_argument(
        "--inference-timesteps",
        type=int,
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        help="Local VoxCPM model path (overrides Hub download)",
    )
    parser.add_argument(
        "--hf-model-id",
        type=str,
        default="openbmb/VoxCPM",
        help="Hugging Face repo id (e.g., openbmb/VoxCPM)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--local-files-only",
        action="store_true",
        help="Use only local files (no network)",
    )
    parser.add_argument(
